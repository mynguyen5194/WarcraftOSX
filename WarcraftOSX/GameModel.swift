    ...
    func timestep() throws {
        ...
        for asset in actualMap.assets {
            ...
            if asset.action == .capability {
                ...
                var harvestDirection = asset.tilePosition.adjacentTileDirection(position: tilePosition, objSize: 1)
                ...
                if harvestDirection == .max {
                    ...
                    if harvestSteps <= asset.step {
                        ...
                        if 0 >= lumberAvailable[tilePosition.y][tilePosition.x] {
                            actualMap.changeTileType(position: tilePosition, to: .stump)
                        }
                ...
                let tilePosition = Position()
                let mineDirection = asset.tilePosition.adjacentTileDirection(position: tilePosition, objSize: 1)
                ...
                if currentTarget.isAlive {
                    let repairDirection = asset.tilePosition.adjacentTileDirection(position: currentTarget.tilePosition, objSize: currentTarget.size)
                ...
                if asset.type == .none {
                    ...
                    if asset.effectiveRange == 1 {
                        let attackDirection = asset.tilePosition.adjacentTileDirection(position: currentCommand.assetTarget!.tilePosition, objSize: currentCommand.assetTarget!.size)
            ...
            if asset.action == .walk {
                if asset.tileAligned {
                    ...
                    if travelDirection != .max {
                        ...
                        if tilePosition == asset.tilePosition || asset.tilePosition.adjacentTileDirection(position: tilePosition, objSize: 1) != .max {
                            asset.popCommand()
                            asset.resetStep()
                            continue
