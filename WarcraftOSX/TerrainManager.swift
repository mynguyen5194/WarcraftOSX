...
class TerrainManager {
    ...
    // For splitting a sprite sheet (input as CGImage) into numSprites different textures, returned as [SKTexture]
    private func splitVerticalSpriteSheet(image: CGImage, numSprites: Int) -> [SKTexture] {
        
        let segmentHeight: CGFloat = CGFloat.init(image.height) / CGFloat(numSprites)
        var cropRect: CGRect = CGRect(x: 0, y: 0, width: image.width, height: Int(segmentHeight))
        ...
        for i in 0 ..< numSprites {
            ...
            let currentSegmentCGImage = image.cropping(to: cropRect)
            //let currentSegmentUIImage = UIImage(cgImage: currentSegmentCGImage!)
            let currentSegmentSKTexture = SKTexture(image: currentSegmentCGImage)
